...
---Fill items with currently equipped items AND bag/bank items for combined export.
function EquipmentSpecMeta:UpdateAllItems(unit)
    -- First populate equipped items (slots 1-17)
    self:UpdateEquippedItems(unit)
    
    -- Then append bag/bank items without resetting
    local GetContainerNumSlots = C_Container.GetContainerNumSlots
    local GetContainerItemLink = C_Container.GetContainerItemLink
    
    -- armssim patch: scan carried bags AND the bank.
    -- Bank containers only return items while the bank frame is open
    -- (Blizzard API limitation); when it's closed these yield 0 slots, so this
    -- is a safe no-op away from the bank.
    local containers = {}
    for bagId = 0, NUM_BAG_SLOTS do
        table.insert(containers, bagId)
    end
    table.insert(containers, BANK_CONTAINER or -1)                 -- main bank window slots
    local numBankBags = NUM_BANKBAGSLOTS or 7
    for bagId = NUM_BAG_SLOTS + 1, NUM_BAG_SLOTS + numBankBags do  -- purchased bank bags
        table.insert(containers, bagId)
    end
